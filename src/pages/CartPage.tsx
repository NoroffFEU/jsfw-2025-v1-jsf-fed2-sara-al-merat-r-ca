      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          {cart.map((item) => (
            <div
              key={item.id}
              className="flex flex-col sm:flex-row bg-white rounded-2xl border-2 border-gray-200 p-4 gap-6 items-center shadow-sm">
              {/* Product Thumbnail */}
              ...
              <div className="flex flex-col grow gap-4 w-full">
                ...
                <div className="flex justify-between items-start">
                  <Link
                    to={`/product/${item.id}`}
                    className="text-xl font-bold text-indigo-950 hover:text-orange-500 transition-colors line-clamp-1">
                    {item.title}
                  </Link>
                  <button
